...
def Initialization(settings):
    ...
    p.info("gestureRecognizerを初期化中")

    p.success("gestureRecognizerの初期化完了")

    return True

def run(hand_landmarks):
    if not hand_landmarks:
        p.debug("認識対象の手がありません")
    ...
    landmarks = hand_landmarks[0].get("landmarks", [])

    return recognize(landmarks)
