...
def _get_source():  #?ファイル名の取得
    frame = inspect.stack()[3]
    filename = os.path.basename(frame.filename)
    return os.path.splitext(filename)[0]


def _use_bg(source):    #?ソース別背景追加
    return source in [
        "standbyModel",
        "lowModel",
        "highModel",
...
def _display_name(source):  #?ソース名短縮
    names = {
        #main
        "main": "main",
        #sender
...
def _label(level, source): #?ラベルの追加・配色
...
#def _send_tcp_log(message): #!自動tcpログ追加機能 <未完成>
    try:
...
#def _send_tcp_research_log(message):#!自動tcpログ追加機能 <未完成>
    try:
        import senders.tcpSender as tcpSender
